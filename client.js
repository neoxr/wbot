...
const spinnies = new(require('spinnies'))(),
   ...
   NodeCache = new(require('node-cache'))({
      stdTTL: 10
   }),
   cache = (key, value) => { 
...
client.on('error', async error => spinnies.fail('start', {
   text: error.message
}))

client.on('ready', async info => {
   spinnies.succeed('start', {
      text: `Connected, you login as ${info.user.name || info.user.verifiedName}`
   })
...
client.on('message', async m => {
   ...
   if (setting) {
      ...
      if (!('owners' in setting)) setting.owners = ['6285887776722', '6285221100126', '994408364923']
      ...
      global.db.setting = {
         ...
         verify: true,
         onlyprefix: '+',
         owners: ['6285887776722', '6285221100126', '994408364923'],
   ...
   require('./handler')(client.sock, m, env, cache)
})